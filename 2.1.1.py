...
class DataSet:
    ...
    def get_average_sum_by_dic(dic):
        ...
        for key, values in dic.items():
            temp[key] = int(sum(values) / len(values))
        return temp

    @staticmethod
    def increase_value(dic, key, value):
        if key in dic:
            dic[key] += value
        else:
            dic[key] = value
    ...
    def get_statistic(self):
        ...
        counter, salary_of_vac, num_vac, name_num_vac = self.statistics_helper(area_salary, 0, salary, salary_of_vac)

        stats = self.get_average_sum_by_dic(salary)
        s2 = self.get_average_sum_by_dic(salary_of_vac)
        s3 = self.get_average_sum_by_dic(area_salary)

        s4 = {}
        for year, salaries in area_salary.items():
            s4[year] = round(len(salaries) / counter, 4)
        s4 = list(filter(lambda a: a[-1] >= 0.01, [(key, value) for key, value in s4.items()]))
        s4.sort(key=lambda a: a[-1], reverse=True)
        s5 = s4.copy()
        s4 = dict(s4)
        s3 = list(filter(lambda a: a[0] in list(s4.keys()), [(key, value) for key, value in s3.items()]))
        s3.sort(key=lambda a: a[-1], reverse=True)
        s3 = dict(s3[:10])
        s5 = dict(s5[:10])

        return stats, num_vac, s2, name_num_vac, s3, s5
    ...
    def statistics_helper(self, area_salary, counter, salary, salary_of_vac):
        ...
        return counter, salary_of_vac, num_vac, name_num_vac

    @staticmethod
    def print_result(s1, s2, s3, s4, s5, s6):
...
class InputConnect:
    def __init__(self):
        ...
        dataset = DataSet(self.file, self.vacancy)
        s1, s2, s3, s4, s5, s6 = dataset.get_statistic()
        dataset.print_result(s1, s2, s3, s4, s5, s6)
...
class Report:
    ...
    def generate_excel(self):
        ...
        for index, _ in enumerate(self.s5):
            second['E' + str(index + 2)].number_format = '0.00%'
        ...
        for element, _ in enumerate(self.s1):
            for e in letters[0]:
